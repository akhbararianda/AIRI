...
bool enforcement(){auto e=env(L"AIRI_LICENSE_CLOUD_ENFORCE");if(!e.empty())return e==L"1"||e==L"true"||e==L"TRUE";if(reg_flag(L"Enforcement"))return true;auto p=appdir()/L"airi-cloud.enforce";return std::filesystem::exists(p);}
...
std::string os_version(){OSVERSIONINFOW v{};v.dwOSVersionInfoSize=sizeof(v);#pragma warning(push)
#pragma warning(disable:4996)
BOOL ok=GetVersionExW(&v);
#pragma warning(pop)
if(!ok)return"Windows";return"Windows "+std::to_string(v.dwMajorVersion)+"."+std::to_string(v.dwMinorVersion)+" build "+std::to_string(v.dwBuildNumber);}
