    ...
    std::thread([id,r=std::move(r)]()mutable{
        bool ok=airi::MediaWorker::download(r,[id](const airi::MediaProgress&p){
            {std::lock_guard lk(g_media_mtx);for(auto&j:g_media)if(j.id==id){
                if(!p.status.empty())j.status=W(p.status);if(p.percent>=0)j.progress=std::clamp(p.percent,0,100);if(!p.downloaded_text.empty())j.downloaded=W(p.downloaded_text);if(!p.total_text.empty())j.total=W(p.total_text);if(!p.speed_text.empty())j.speed=W(p.speed_text);if(!p.eta_text.empty())j.eta=W(p.eta_text);if(!p.fragment_text.empty())j.fragment=W(p.fragment_text);if(!p.output_path.empty())j.output_path=std::filesystem::path(W(p.output_path));j.done=p.status=="Completed";if(p.status=="Failed")j.failed=true;if(p.status=="Cancelled")j.cancelled=true;break;
            }}if(g_hwnd)PostMessageW(g_hwnd,WM_REFRESH,0,0);
        });
        timing_finish(id);bool wasCancelled=r.cancel_requested&&r.cancel_requested->load();notice(ok?L"Media completed":wasCancelled?L"Media cancelled":L"Media failed");
    }).detach();return true;
}
...
void buttons(){
    ...
    if(id&&media_id(id)){canCancel=true;}
    else if(id){if(auto t=g_mgr->find(id)){auto s=t->snapshot();canPause=s.status==airi::DownloadStatus::Downloading;canResume=s.status==airi::DownloadStatus::Paused;canCancel=!terminal(s.status);}}
    EnableWindow(g_pause,canPause);EnableWindow(g_resume,canResume);EnableWindow(g_cancel,canCancel);EnableWindow(g_open,TRUE);for(HWND x:{g_pause,g_resume,g_cancel,g_open})InvalidateRect(x,nullptr,TRUE);
}
...
void refresh(){
    ...
    for(auto&s:g_mgr->snapshots()){
        if(terminal(s.status))timing_finish(s.id);auto name=W(s.filename.empty()?"download":s.filename),st=W(airi::status_name(s.status));if(s.status==airi::DownloadStatus::Downloading){g_active++;g_total_speed+=(unsigned long long)s.bytes_per_second;}if(s.status==airi::DownloadStatus::Completed)g_completed++;if(s.status==airi::DownloadStatus::Failed)g_failed++;if(!matches_filter(name,st))continue;LVITEMW i{};i.mask=LVIF_TEXT|LVIF_PARAM;i.iItem=row;i.pszText=name.data();i.lParam=(LPARAM)s.id;int r=ListView_InsertItem(g_list,&i);setcell(r,1,st);setcell(r,2,std::to_wstring(s.progress_percent)+L"%");setcell(r,3,s.bytes_per_second>0?W(airi::text::human_bytes((unsigned long long)s.bytes_per_second))+L"/s":L"-");auto size=W(airi::text::human_bytes(s.downloaded_bytes))+L" / "+W(airi::text::human_bytes(s.total_bytes));setcell(r,4,size);setcell(r,5,eta_text(s.total_bytes,s.downloaded_bytes,s.bytes_per_second));auto t=timing_get(s.id);setcell(r,6,fmt_time(t.started,t.has_start));setcell(r,7,fmt_elapsed(t));if(s.id==sel)ListView_SetItemState(g_list,r,LVIS_SELECTED|LVIS_FOCUSED,LVIS_SELECTED|LVIS_FOCUSED);row++;
    }
    {std::lock_guard lk(g_media_mtx);for(auto&j:g_media){if(!j.done&&!j.failed&&!j.cancelled){g_active++;g_total_speed+=speed_bytes(j.speed);}if(j.done)g_completed++;if(j.failed)g_failed++;if(!matches_filter(j.name,j.status))continue;LVITEMW i{};i.mask=LVIF_TEXT|LVIF_PARAM;i.iItem=row;i.pszText=j.name.data();i.lParam=(LPARAM)j.id;int r=ListView_InsertItem(g_list,&i);setcell(r,1,j.status);setcell(r,2,std::to_wstring(j.progress)+L"%");setcell(r,3,j.speed.empty()?L"-":j.speed);std::wstring sz=j.downloaded;if(!j.total.empty()&&j.total!=L"NA")sz+=(sz.empty()?L"":L" / ")+j.total;setcell(r,4,sz.empty()?L"-":sz);setcell(r,5,j.eta.empty()?L"-":j.eta);auto t=timing_get(j.id);setcell(r,6,fmt_time(t.started,t.has_start));setcell(r,7,fmt_elapsed(t));if(j.id==sel)ListView_SetItemState(g_list,r,LVIS_SELECTED|LVIS_FOCUSED,LVIS_SELECTED|LVIS_FOCUSED);row++;}}
    InvalidateRect(g_hwnd,nullptr,FALSE);buttons();
}

void open_folder(){auto d=downloads();auto id=selected();if(id&&media_id(id)){std::lock_guard lk(g_media_mtx);for(auto&j:g_media)if(j.id==id){d=!j.output_path.empty()?j.output_path.parent_path():j.dir;break;}}else if(id){if(auto t=g_mgr->find(id)){auto s=t->snapshot();if(!s.output_path.empty())d=s.output_path.parent_path();}}ShellExecuteW(g_hwnd,L"open",d.c_str(),nullptr,nullptr,SW_SHOWNORMAL);}
void open_selected(){auto id=selected();if(!id){open_folder();return;}if(media_id(id)){std::filesystem::path p;{std::lock_guard lk(g_media_mtx);for(auto&j:g_media)if(j.id==id){p=j.output_path;break;}}if(!p.empty()&&std::filesystem::exists(p)){ShellExecuteW(g_hwnd,L"open",p.c_str(),nullptr,nullptr,SW_SHOWNORMAL);return;}open_folder();return;}if(auto t=g_mgr->find(id)){auto s=t->snapshot();if(!s.output_path.empty()&&std::filesystem::exists(s.output_path)){ShellExecuteW(g_hwnd,L"open",s.output_path.c_str(),nullptr,nullptr,SW_SHOWNORMAL);return;}}open_folder();}
void cancel_media(std::uint64_t id){std::lock_guard lk(g_media_mtx);for(auto&j:g_media)if(j.id==id&&!j.done&&!j.failed&&!j.cancelled){if(j.cancel)j.cancel->store(true);j.status=L"Cancelling...";break;}notice(L"Cancelling media download...");}
...
LRESULT custom_draw(NMLVCUSTOMDRAW*cd){if(cd->nmcd.dwDrawStage==CDDS_PREPAINT)return CDRF_NOTIFYITEMDRAW;if(cd->nmcd.dwDrawStage==CDDS_ITEMPREPAINT){cd->clrText=C_TEXT;cd->clrTextBk=(cd->nmcd.dwItemSpec%2)?C_SURFACE:C_SURFACE2;return CDRF_NOTIFYSUBITEMDRAW;}if(cd->nmcd.dwDrawStage==(CDDS_ITEMPREPAINT|CDDS_SUBITEM)){if(cd->iSubItem==2){RECT rc{};ListView_GetSubItemRect(g_list,(int)cd->nmcd.dwItemSpec,2,LVIR_BOUNDS,&rc);wchar_t txt[32]{};ListView_GetItemText(g_list,(int)cd->nmcd.dwItemSpec,2,txt,32);int pct=std::clamp(_wtoi(txt),0,100);round_fill(cd->nmcd.hdc,RECT{rc.left+8,rc.top+8,rc.right-8,rc.bottom-8},RGB(225,235,245),8);RECT fill{rc.left+8,rc.top+8,rc.left+8+(rc.right-rc.left-16)*pct/100,rc.bottom-8};if(fill.right>fill.left)round_fill(cd->nmcd.hdc,fill,C_SKY,8);draw_text(cd->nmcd.hdc,txt,rc,C_TEXT,g_font_bold,DT_CENTER|DT_VCENTER|DT_SINGLELINE);return CDRF_SKIPDEFAULT;}return CDRF_DODEFAULT;}return CDRF_DODEFAULT;}
...
case WM_CREATE:{g_hwnd=h;BOOL dark=FALSE;DwmSetWindowAttribute(h,20,&dark,sizeof(dark));auto mkbtn=[&](HWND&out,int id,const wchar_t*t){out=CreateWindowW(L"BUTTON",t,WS_CHILD|WS_VISIBLE|WS_TABSTOP|BS_OWNERDRAW,0,0,100,36,h,(HMENU)(INT_PTR)id,nullptr,nullptr);SendMessageW(out,WM_SETFONT,(WPARAM)g_font_bold,TRUE);};mkbtn(g_add,ID_ADD,L"+ New");mkbtn(g_resume,ID_RESUME,L"Resume");mkbtn(g_pause,ID_PAUSE,L"Pause");mkbtn(g_cancel,ID_CANCEL,L"Cancel");mkbtn(g_open,ID_OPEN,L"Open file");mkbtn(g_pause_all,ID_PAUSE_ALL,L"Pause all");mkbtn(g_resume_all,ID_RESUME_ALL,L"Resume all");g_search=CreateWindowExW(WS_EX_CLIENTEDGE,L"EDIT",L"",WS_CHILD|WS_VISIBLE|WS_TABSTOP|ES_AUTOHSCROLL,0,0,200,36,h,(HMENU)(INT_PTR)ID_SEARCH,nullptr,nullptr);SendMessageW(g_search,EM_SETCUEBANNER,TRUE,(LPARAM)L"Search downloads...");g_filter=CreateWindowW(WC_COMBOBOXW,L"",WS_CHILD|WS_VISIBLE|CBS_DROPDOWNLIST,0,0,150,200,h,(HMENU)(INT_PTR)ID_FILTER,nullptr,nullptr);for(auto x:{L"All downloads",L"Active",L"Completed",L"Paused",L"Failed",L"Cancelled"})SendMessageW(g_filter,CB_ADDSTRING,0,(LPARAM)x);SendMessageW(g_filter,CB_SETCURSEL,0,0);g_list=CreateWindowExW(0,WC_LISTVIEWW,L"",WS_CHILD|WS_VISIBLE|LVS_REPORT|LVS_SINGLESEL|LVS_SHOWSELALWAYS,0,0,800,400,h,(HMENU)(INT_PTR)ID_LIST,nullptr,nullptr);ListView_SetExtendedListViewStyle(g_list,LVS_EX_FULLROWSELECT|LVS_EX_DOUBLEBUFFER|LVS_EX_LABELTIP);ListView_SetBkColor(g_list,C_SURFACE);ListView_SetTextBkColor(g_list,C_SURFACE);ListView_SetTextColor(g_list,C_TEXT);SendMessageW(g_list,WM_SETFONT,(WPARAM)g_font,TRUE);struct Col{const wchar_t*t;int width;}cs[]={{L"Name",300},{L"Status",118},{L"Progress",150},{L"Speed",110},{L"Downloaded / Total",170},{L"ETA",92},{L"Started",118},{L"Elapsed",94}};for(int i=0;i<8;i++){LVCOLUMNW c{};c.mask=LVCF_TEXT|LVCF_WIDTH;c.pszText=(LPWSTR)cs[i].t;c.cx=cs[i].width;ListView_InsertColumn(g_list,i,&c);}SendMessageW(g_search,WM_SETFONT,(WPARAM)g_font,TRUE);SendMessageW(g_filter,WM_SETFONT,(WPARAM)g_font,TRUE);SetTimer(h,1,500,nullptr);layout(h);buttons();return 0;}
...
case WM_PAINT:{PAINTSTRUCT ps{};HDC dc=BeginPaint(h,&ps);RECT cr{};GetClientRect(h,&cr);FillRect(dc,&cr,g_bg_brush);draw_text(dc,L"AIRI Download Manager",RECT{24,16,700,52},C_TEXT,g_title_font);draw_text(dc,L"Smart Turbo + Universal Media Center  •  v2.6 AIRI Edition",RECT{26,52,850,78},C_MUTED,g_font);int gap=12,cw=((int)std::max<LONG>(900,cr.right)-48-gap*3)/4,x=24;draw_card(dc,RECT{x,88,x+cw,142},L"ACTIVE",std::to_wstring(g_active),C_ACCENT);x+=cw+gap;draw_card(dc,RECT{x,88,x+cw,142},L"TOTAL SPEED",W(airi::text::human_bytes(g_total_speed))+L"/s",C_SKY);x+=cw+gap;draw_card(dc,RECT{x,88,x+cw,142},L"COMPLETED",std::to_wstring(g_completed),C_GREEN);x+=cw+gap;draw_card(dc,RECT{x,88,x+cw,142},L"FAILED",std::to_wstring(g_failed),g_failed?C_RED:C_MUTED);std::wstring nt;{std::lock_guard lk(g_notice_mtx);nt=g_notice;}draw_text(dc,(L"Developed by AIRI Technology  •  Founder Akhbar Arianda  •  "+nt).c_str(),RECT{24,cr.bottom-46,cr.right-24,cr.bottom-12},C_MUTED,g_font);EndPaint(h,&ps);return 0;}
...
case WM_COMMAND:{int id=LOWORD(wp);if(id==ID_SEARCH&&HIWORD(wp)==EN_CHANGE){refresh();return 0;}if(id==ID_FILTER&&HIWORD(wp)==CBN_SELCHANGE){refresh();return 0;}if(id==ID_ADD&&HIWORD(wp)==BN_CLICKED){AddData d;auto r=DialogBoxParamW(GetModuleHandleW(nullptr),MAKEINTRESOURCEW(IDD_ADD_URL),h,AddDlg,(LPARAM)&d);if(r==IDOK)add_direct(U8(d.url),U8(d.filename),{},{},{},{},d.connections);return 0;}if(id==ID_OPEN&&HIWORD(wp)==BN_CLICKED){open_selected();return 0;}if(id==ID_PAUSE_ALL&&HIWORD(wp)==BN_CLICKED){act_all(false);return 0;}if(id==ID_RESUME_ALL&&HIWORD(wp)==BN_CLICKED){act_all(true);return 0;}if((id==ID_PAUSE||id==ID_RESUME||id==ID_CANCEL)&&HIWORD(wp)==BN_CLICKED){auto sid=selected();if(!sid){notice(L"Select a download first");return 0;}if(media_id(sid)){if(id==ID_CANCEL)cancel_media(sid);else notice(L"Pause/Resume for segmented media will be added in the next engine revision");refresh();return 0;}auto t=g_mgr->find(sid);if(!t)return 0;if(id==ID_PAUSE)t->pause();else if(id==ID_RESUME)t->resume();else{t->cancel();timing_finish(sid);}refresh();return 0;}return 0;}
case WM_DESTROY:KillTimer(h,1);g_hwnd=nullptr;PostQuitMessage(0);return 0;
}return DefWindowProcW(h,m,wp,lp);}
}

int WINAPI wWinMain(HINSTANCE hi,HINSTANCE,PWSTR,int show){SetProcessDPIAware();CoInitializeEx(nullptr,COINIT_APARTMENTTHREADED);INITCOMMONCONTROLSEX ic{sizeof(ic),ICC_LISTVIEW_CLASSES|ICC_STANDARD_CLASSES};InitCommonControlsEx(&ic);NONCLIENTMETRICSW n{sizeof(n)};SystemParametersInfoW(SPI_GETNONCLIENTMETRICS,sizeof(n),&n,0);LOGFONTW lf=n.lfMessageFont;wcscpy_s(lf.lfFaceName,L"Segoe UI");lf.lfHeight=-15;g_font=CreateFontIndirectW(&lf);lf.lfWeight=FW_SEMIBOLD;g_font_bold=CreateFontIndirectW(&lf);lf.lfHeight=-28;lf.lfWeight=FW_BOLD;g_title_font=CreateFontIndirectW(&lf);lf.lfHeight=-22;g_stat_font=CreateFontIndirectW(&lf);g_bg_brush=CreateSolidBrush(C_BG);g_surface_brush=CreateSolidBrush(C_SURFACE2);g_icon=LoadIconW(hi,MAKEINTRESOURCEW(IDI_AIRI));g_mgr=std::make_unique<airi::DownloadManager>([](const airi::DownloadSnapshot&){if(g_hwnd)PostMessageW(g_hwnd,WM_REFRESH,0,0);});register_bridge();WNDCLASSEXW wc{sizeof(wc)};wc.lpfnWndProc=Proc;wc.hInstance=hi;wc.lpszClassName=kClass;wc.hCursor=LoadCursorW(nullptr,IDC_ARROW);wc.hIcon=g_icon?g_icon:LoadIconW(nullptr,IDI_APPLICATION);wc.hIconSm=wc.hIcon;wc.hbrBackground=g_bg_brush;RegisterClassExW(&wc);auto h=CreateWindowExW(0,kClass,L"AIRI Download Manager v2.6 AIRI Edition",WS_OVERLAPPEDWINDOW|WS_CLIPCHILDREN,CW_USEDEFAULT,CW_USEDEFAULT,1400,840,nullptr,nullptr,hi,nullptr);if(!h)return 2;ShowWindow(h,show);UpdateWindow(h);MSG msg{};while(GetMessageW(&msg,nullptr,0,0)>0){TranslateMessage(&msg);DispatchMessageW(&msg);}g_mgr.reset();DeleteObject(g_font);DeleteObject(g_font_bold);DeleteObject(g_title_font);DeleteObject(g_stat_font);DeleteObject(g_bg_brush);DeleteObject(g_surface_brush);CoUninitialize();return(int)msg.wParam;}
#endif
