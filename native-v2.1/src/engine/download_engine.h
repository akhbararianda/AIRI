...
class DownloadTask:public std::enable_shared_from_this<DownloadTask>{public:using Callback=std::function<void(const DownloadSnapshot&)>;DownloadTask(std::uint64_t,DownloadRequest,Callback);~DownloadTask();void start();void pause();void resume();void cancel();DownloadSnapshot snapshot()const;private:void run();bool probe();bool segmented();bool sequential();void update_speed();void emit();void fail(std::string);std::uint64_t id_{};DownloadRequest req_;Callback cb_;mutable std::mutex m_;std::mutex file_m_;std::thread th_;std::atomic<bool> paused_{false},cancelled_{false};std::atomic<std::uint64_t> downloaded_{0};DownloadSnapshot s_;std::uint64_t chunk_=4ull*1024*1024;bool ranges_=false;};
