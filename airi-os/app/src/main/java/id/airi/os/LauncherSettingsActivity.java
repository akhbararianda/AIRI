...
public class LauncherSettingsActivity extends Activity {
    ...
    private void build(){ScrollView sc=new ScrollView(this);root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(38),dp(18),dp(40));root.setBackgroundColor(AiriTheme.surface(this));sc.addView(root,new ScrollView.LayoutParams(-1,-2));
        TextView title=t("Fusion Launcher",30,true);root.addView(title);TextView sub=t("AIRI v19 • launcher controls",12,false);sub.setTextColor(AiriTheme.muted(this));root.addView(sub,lp(-1,-2,0,2,0,18));
        toggle("Predictive suggestions","Prioritize apps you launch most",LauncherPrefs.suggestions(this),v->LauncherPrefs.suggestions(this,v));
        ...
        toggle("Compact mode","Smaller tiles for faster scanning",LauncherPrefs.compact(this),v->LauncherPrefs.compact(this,v));
        ...
        action("Reset launcher preferences","Reset grid, labels, categories, favorites and hidden apps",()->{LauncherPrefs.reset(this);Toast.makeText(this,"Fusion preferences reset",Toast.LENGTH_SHORT).show();recreate();});
