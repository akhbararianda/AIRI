...
public class IntelligenceHubActivity extends Activity {
    ...
    private void build(){ScrollView sc=new ScrollView(this);LinearLayout root=new LinearLayout(this);root.setOrientation(LinearLayout.VERTICAL);root.setPadding(dp(18),dp(28),dp(18),dp(28));root.setBackgroundColor(Color.rgb(226,237,246));sc.addView(root,new ScrollView.LayoutParams(-1,-2));
        root.addView(t("AIRI Intelligence",30,true));TextView sub=t("Stable Complete • Android 11 • True Liquid",13,false);sub.setTextColor(Color.rgb(72,91,108));root.addView(sub,lp(-1,-2,0,2,0,16));
        ...
        TextView note=t("AIRI OS v8 Stable keeps the launcher lightweight and routes system features through official Android APIs and user-approved permissions. OEM Recents and status bar remain outside an ordinary APK launcher.",12,false);note.setTextColor(Color.rgb(72,91,106));note.setPadding(dp(15),dp(14),dp(15),dp(14));note.setBackground(AiriGlassDrawable.make(this,26,AiriGlassDrawable.CLEAR));root.addView(note,lp(-1,-2,0,16,0,0));setContentView(sc);AiriLiquidSkin.apply(this);}
